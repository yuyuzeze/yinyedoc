# 政府内部システム移行プロジェクト — プロジェクトコンテキスト定義書 v8

> **本文書の目的**: プロジェクトの業務概要・技術スタック・アーキテクチャ・API設計方針を一元管理する。
> コード生成・設計レビュー・ドキュメント作成の際は本文書を最優先の前提情報として参照すること。
> **更新履歴**: v7（第2段階PaaS廃止確定 + .NET 10移行 + 認証再設計）→ **v8（Angular 20確定 + 実際のリポジトリ構成・フォルダ構造・Middlewareを実コードに同期）**
...
| **目的** | 既存 ASP.NET Web Forms + VB.NET システムを Angular 20 + **.NET 10** に全面刷新 |
...
> **リポジトリ**: `linyefrontend`（ルート直下に `src/`・`angular.json`・`package.json` 等）

| 項目 | 採用技術 | バージョン | 備考 |
|------|---------|---------|------|
| フレームワーク | Angular | **20.0.x** | ★v8更新: 18→20確定（package.json実測値） |
| 言語 | TypeScript | 5.8.x | |
| UI ライブラリ | Angular Material + CDK | 20.0.x | Material 3 デザイン |
| 状態管理 | Angular Signals | — | RxJS と併用 |
| HTTP 通信 | HttpClient + HttpInterceptor | — | MsalInterceptor / ErrorInterceptor / LoadingInterceptor / LoggingInterceptor |
| ルーティング | @angular/router | 20.0.x | Lazy Loading（サブシステム単位） |
| フォーム | Reactive Forms | — | ReactiveFormsModule |
| 認証 | @azure/msal-angular + @azure/msal-browser | 3.x | Entra ID SSO。Authorization Code Flow + PKCE。`environment.auth.enabled` トグルで開発時モック可 |
...
| ファイル | 役割 |
|---------|------|
| `core/auth/` MsalInterceptor | Entra ID Access Token を全リクエストに自動付与 |
| `core/interceptors/error.interceptor.ts` | 401→MSAL再ログイン / 403→forbidden画面 / 500→エラートースト |
| `core/interceptors/loading.interceptor.ts` | リクエスト中のプログレスバー自動表示 |
| `core/logging/logging.interceptor.ts` | リクエストログをサーバーへ送信 |
...
❸ Swagger / SwaggerUI              → 開発時のみ
❹ CORS                             → 開発時のみ（localhost:4200）
❺ Routing
❻ Authentication                   → Microsoft.Identity.Web が Entra ID JWT を検証
❼ RequestLogContextMiddleware      → ログコンテキスト（UPN 等）付与
❽ RoleMiddleware                   → UserRole テーブルでロール確認
❾ Authorization
...
`ApiResponse<T>` + `MessageContainer`（各リストは `ApiMessageItem` オブジェクト）

```json
{
  "result": {},
  "messages": {
    "nrmList": [{ "code": "IHOSHO20001", "message": "処理が完了しました。", "params": [] }],
    "wrnList": [],
    "errList": []
  },
...
```typescript
// TypeScript / C# 共通構造
{ code: string; message: string; params?: (string | number)[]; }
```
...
### 6.1 Angular 20 フロントエンド（リポジトリ: linyefrontend）
...
│   │   ├── core/
│   │   │   ├── auth/
│   │   │   │   ├── msal-config.ts              # MSALInstanceFactory / MSALGuardConfigFactory / MSALInterceptorConfigFactory
...
│       └── environment.development.ts # 開発（auth.enabled=false でモック可）
├── proxy.conf.json                     # /api → localhost:5001 に転送
└── angular.json
```

### 6.2 .NET 10 バックエンド（リポジトリ: linyebackend）

```
（リポジトリルート）
├── Nogyosuisan.sln
└── src/
    ├── Api/                             # Web API エントリポイント
    │   ├── Controllers/
    │   │   ├── Hosho/
    │   │   ├── Satei/
    │   │   ├── Suishin/
    ...
    │   │   ├── RequestLogContextMiddleware.cs  # ログコンテキスト（UPN等）付与
    │   │   └── RoleMiddleware.cs              # UserRole テーブルでロール確認
    │   ├── Models/
    ...
    │   └── Program.cs
    ├── Application/                     # 応用層（DTO・サービスインターフェース/実装）
    │   ├── Services/
    │   │   └── Report/
    │   └── DTOs/
    ├── Domain/                          # ドメイン層（エンティティ）
    │   └── Entities/
    └── Infrastructure/                  # インフラ層（EF Core・Repository）
        ├── Data/
        │   ├── AppDbContext.cs
        │   └── Repositories/
        ├── Reports/
...
## 8. npm パッケージ（Angular 20）

| パッケージ | バージョン | 用途 |
|-----------|---------|------|
| @angular/* | **20.0.x** | Angular コア一式（★v8: 18→20確定） |
| @azure/msal-angular | 3.x | Entra ID SSO（Angular 統合） |
| @azure/msal-browser | 3.x | MSAL ブラウザコア |
| rxjs | 7.8.x | リアクティブプログラミング |
| typescript | 5.8.x | TypeScript コンパイラ（★v8: 5.4→5.8） |
...
npm install
ng serve
# → http://localhost:4200  /api → localhost:5001 に自動転送（proxy.conf.json）
```

```json
// proxy.conf.json
{ "/api": { "target": "http://localhost:5001", "secure": false, "changeOrigin": true } }
```

**認証OFF開発モード**: `environment.development.ts` で `auth.enabled = false` に設定すると  
MsalGuard をスキップし `AuthContextService.ensureLoaded()` で `/api/auth/me` からユーザー情報取得。

**テスト環境**: 個人 Azure テスト環境 + ローカル Windows Server
...
| 選定 | 理由 |
|------|------|
| Angular **20**（not React/Vue） | 100画面の政府系システムに統一規範が最適。VPN内部システムで脱保リスク低 |
...
*本文書バージョン: **v8***
*更新内容: v7 → v8*
- *Angular 18 → **Angular 20** 確定（package.json実測値）*
- *フロントエンドリポジトリ: `linyefrontend`、バックエンドリポジトリ: `linyebackend`（Nogyosuisan.sln）に更新*
- *フォルダ構造を実コードに同期: `core/auth/`・`core/logging/`・`core/messages/`・`core/utils/` 追加*
- *Middleware Pipeline に `RequestLogContextMiddleware`・`RoleMiddleware` 追加*
- *`MessageContainer` の各リストを `List<string>` → `List<ApiMessageItem>`（code/message/params）に修正*
- *npm パッケージバージョンを実測値（TypeScript 5.8.x 等）に更新*
- *開発環境パスを実際のリポジトリ構成に更新*
- *`environment.auth.enabled` トグルによる開発モック方式を追記*
